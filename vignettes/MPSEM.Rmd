...
Maps (PEM) as implemented in R package MPSEM ($0.2$-$6$).}
...
estimate a single value of parameter $a$ for the whole phylogeny%
\footnote{Function {\tt PEM.fitSimple()} does not estimate parameter $\psi$
...
\footnote{A function to estimate different sets of weighting function parameters
for different portions of the phylogeny has yet to be included in
{\tt MPSEM}.%
}. That function requires a response variable that will be used to

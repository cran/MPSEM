...
Maps (PEM) as implemented in R package MPSEM ($0.3$-$1$).}
...
\protect\caption{Values of the edge weighting function used as a model of trait evolution
...
\protect\caption{Predicted and observed $\log_{10}$ neonatal body mass for \Sexpr{nrow(perissodactyla.data)}
